...
- 엑셀 다운로드는 외부 엑셀 처리 라이브러리 없이 사용안내, 입력값, 붙여넣기용, 계산결과 시트를 포함한 `.xlsx` 파일을 브라우저에서 생성합니다.
- 엑셀 업로드 기능은 제거했고, 엑셀에서 복사한 표 데이터를 붙여넣으면 현재 브라우저에서만 해석해 입력값을 복원합니다.
- 붙여넣기 데이터는 길이와 행 수를 제한하고, 다른 계산기 ID가 들어 있는 데이터는 반영하지 않습니다.
...
- 엑셀 다운로드와 붙여넣기 해석은 클라이언트 브라우저에서만 처리하며 서버에 파일이나 표 데이터를 전송하지 않음
