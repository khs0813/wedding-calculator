      ...
      {items.map((item, index) => {
        ...
        return (
          <div key={item.question} className="py-5">
            <button
              type="button"
              className="flex w-full cursor-pointer items-center justify-between gap-4 text-left text-base font-semibold text-foreground"
              onClick={() => toggle(item.question)}
              aria-expanded={open}
              aria-controls={answerId}
            >
              <span className="faq-question min-w-0 flex-1">{item.question}</span>
              <span className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-secondary text-lg font-semibold text-foreground" aria-hidden="true">
                {open ? "-" : "+"}
              </span>
