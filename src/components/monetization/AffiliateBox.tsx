...
export function AffiliateBox({ keys }: { keys: AffiliateKey[] }) {
  ...
  return (
    <Card className="no-print bg-card">
      <CardHeader>
        <p className="text-sm font-semibold uppercase tracking-[0.2em] text-rose-500">제휴</p>
        <h2 className="text-2xl font-semibold text-foreground">관련 준비 항목</h2>
        <p className="text-sm text-muted-foreground">추후 제휴 링크를 연결할 수 있도록 컴포넌트화된 영역입니다.</p>
      </CardHeader>
      <CardContent>
          ...
          {uniqueKeys.map((key) => {
            ...
            return (
              <a
                ...
                className="rounded-2xl border border-rose-100 bg-rose-50/50 p-5 transition hover:border-rose-200 hover:bg-rose-50"
              >
