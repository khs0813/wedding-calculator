    ...
    <div className="space-y-6">
      <section className="grid gap-3 md:grid-cols-3" aria-label="핵심 지표">
        {keySummaries.map((summary) => (
          <Card key={summary.label} className="p-5 shadow-none">
            <p className="text-sm font-bold text-muted-foreground">{summary.label}</p>
            <p className="mt-2 text-xl font-semibold text-foreground">{summary.value}</p>
            {summary.description ? <p className="mt-2 text-xs leading-5 text-muted-foreground">{summary.description}</p> : null}
          </Card>
        ))}
      </section>
      ...
      {secondarySummaries.length ? (
        <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-3" aria-label="보조 지표">
          {secondarySummaries.map((summary) => (
            <div key={summary.label} className="rounded-2xl border border-border bg-card px-4 py-3">
              <p className="text-sm font-bold text-muted-foreground">{summary.label}</p>
              <p className="mt-1 text-lg font-semibold text-foreground">{summary.value}</p>
              {summary.description ? <p className="mt-2 text-xs leading-5 text-muted-foreground">{summary.description}</p> : null}
            </div>
          ))}
        </section>
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1.05fr)_minmax(20rem,0.95fr)]" aria-label="예산 진단">
        <Card className="overflow-hidden">
          <CardHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xl font-semibold text-foreground">항목별 비중</h3>
                <p className="text-sm text-muted-foreground">총액에서 어디가 가장 큰지 먼저 봅니다.</p>
              </div>
            </div>
          </CardHeader>
          <CardContent className="space-y-4">
            {visibleItems.length > 0 ? (
              ...
              <p className="rounded-2xl bg-secondary p-5 text-sm text-muted-foreground">아직 입력된 비용이 없습니다. 왼쪽 입력 영역에서 금액을 입력해보세요.</p>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="overflow-hidden border-border bg-secondary">
          <CardHeader>
            <div className="flex items-center gap-3">
              <span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-card text-foreground">
                <Lightbulb className="h-5 w-5" aria-hidden="true" />
              </span>
              <div>
                <h3 className="text-xl font-semibold text-foreground">예산 절약 팁</h3>
                <p className="text-sm text-muted-foreground">큰 비중 항목부터 조정합니다.</p>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            <ul className="space-y-3 text-sm leading-7 text-muted-foreground">
              {result.advice.map((tip) => (
                <li key={tip} className="flex gap-3 rounded-2xl bg-card p-3">
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xl font-semibold text-foreground">상세 비용표</h3>
              <p className="text-sm text-muted-foreground">금액이 입력된 항목만 표시됩니다.</p>
            </div>
          </div>
        </CardHeader>
        <CardContent>
                ...
                <tbody>
                  {visibleItems.map((entry) => (
                    <tr key={entry.id} className="border-b border-border last:border-0">
                      <th scope="row" className="py-3 pr-3 text-left font-bold text-foreground">{entry.label}</th>
                      <td className="py-3 pr-3 text-muted-foreground">{entry.category}</td>
                      <td className="py-3 pr-3 text-right font-bold text-foreground">{formatCurrency(entry.amount)}</td>
                      <td className="py-3 text-right text-muted-foreground">{entry.required ? "필수" : "선택"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
